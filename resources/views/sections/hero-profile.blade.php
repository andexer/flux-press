    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div class="flex flex-col md:flex-row items-center gap-16">
            ...
            <div class="text-center md:text-left">
                <flux:badge color="cyan" class="mb-6">{{ __('Disponible para Proyectos', 'sage') }}</flux:badge>
                ...
                <flux:heading size="3xl" class="mb-6 font-medium text-cyan-600 dark:text-cyan-400">
                    {{ __('Diseñador UI & Desarrollador Fullstack.', 'sage') }}
                </flux:heading>
                <flux:text size="xl" class="mb-8 text-zinc-600 dark:text-zinc-400 max-w-xl">
                    {{ __('Ayudo a startups a construir productos digitales escalables y hermosos. Especializado en Laravel, Livewire y Tailwind CSS.', 'sage') }}
                </flux:text>
                
                <div class="flex flex-wrap gap-4 justify-center md:justify-start">
                    <flux:button variant="primary" size="lg" icon="envelope" class="bg-cyan-600 hover:bg-cyan-500 text-white rounded-full px-8">
                        {{ __('Contactar', 'sage') }}
                    </flux:button>
                    <flux:button variant="ghost" size="lg" icon="arrow-down-tray">{{ __('Descargar CV', 'sage') }}</flux:button>
                </div>
            </div>
        </div>
    </div>
