<flux:main class="relative bg-zinc-950 dark:bg-black min-h-[90vh] flex items-center overflow-hidden font-mono">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        <div class="border-l-4 border-red-600 pl-8 py-12">
            <flux:badge color="red" size="sm" class="mb-4 rounded-none uppercase tracking-tighter">{{ __('Nueva Temporada', 'sage') }}</flux:badge>
            ...
            <flux:subheading size="xl" class="mb-10 text-zinc-400 max-w-2xl leading-relaxed">
                {{ __('Únete a la élite competitiva. Hardware, guías y torneos en un solo lugar.', 'sage') }}
            </flux:subheading>
            <div class="flex flex-wrap gap-4">
                <flux:button variant="primary" size="xl" class="bg-red-600 hover:bg-red-700 text-white rounded-none uppercase px-12 border-b-4 border-red-800">
                    {{ __('Jugar Ahora', 'sage') }}
                </flux:button>
                <div class="flex items-center gap-4 ml-4">
                    ...
                    <span class="text-xs text-zinc-500 uppercase">{{ __('+1200 Jugando', 'sage') }}</span>
                </div>
            </div>
        </div>
    </div>
