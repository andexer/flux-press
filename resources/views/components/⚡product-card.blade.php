...
@if ($this->product)
    ...
    <article @class([
        ...
        <div class="relative isolate p-2 pb-0">
            <a href="{{ $product->get_permalink() }}" wire:navigate @class([
                ...
                @if ($product->get_image_id())
                    <img
                        src="{{ wp_get_attachment_image_url($product->get_image_id(), $isCompact ? 'woocommerce_thumbnail' : 'large') }}"
            ...
            <div class="pointer-events-none absolute inset-x-2 top-2 z-10 flex items-start justify-between gap-2">
                ...
                <span class="inline-flex items-center rounded-full bg-red-500 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.08em] text-white shadow-sm">
                    -{{ $discount }}%
                </span>
                @else
                <span></span>
                ...
                @if($product->is_in_stock())
                    <span class="inline-flex items-center rounded-full bg-emerald-500/90 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.08em] text-white shadow-sm">
                        {{ __('Stock', 'flux-press') }}
                    </span>
                @endif
            </div>
            ...
            <div class="pointer-events-none absolute inset-x-2 bottom-2 z-10">
                <div class="inline-flex max-w-full items-center gap-2 rounded-full border border-white/55 bg-zinc-900/55 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-white backdrop-blur-md">
        ...
        <div @class([
            ...
            @if($isShop)
                <div class="mt-2 flex min-h-5 flex-wrap items-center gap-x-2 gap-y-1 text-[11px] font-medium text-zinc-500 dark:text-zinc-400">
                    @if($showSalesMeta)
            ...
            <div class="mt-auto grid gap-2 pt-3.5">
                @if ($product->is_in_stock())
                    ...
                    @else
                        @if($showAddedState)
                            <div class="inline-flex items-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[11px] font-semibold text-emerald-700 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300">
                    ...
                    @endif
                @else
                    <flux:button disabled variant="ghost" class="w-full justify-center rounded-xl border border-zinc-200 bg-zinc-100 h-8 text-[11px] text-zinc-500 cursor-not-allowed dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400">
