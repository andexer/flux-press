...
<section class="py-12 sm:py-14 bg-white dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6 sm:mb-8">
            <flux:badge color="sky" class="mb-2 uppercase tracking-widest">{{ __('Calidad', 'sage') }}</flux:badge>
            <flux:heading size="3xl" class="!font-black tracking-tight">{{ __('Productos mejor valorados', 'sage') }}</flux:heading>
        </div>

        @if(empty($this->products))
            <flux:callout color="zinc" icon="star">
                <flux:callout.heading>{{ __('No hay valoraciones disponibles.', 'sage') }}</flux:callout.heading>
                <flux:callout.text>{{ __('Cuando existan ratings en WooCommerce, esta seccion se llenara automaticamente.', 'sage') }}</flux:callout.text>
            </flux:callout>
        @else
